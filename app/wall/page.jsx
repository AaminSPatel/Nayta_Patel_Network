...
const PostsSection = () => {
  ...
  const fetchCommentsOfPost = async (postId) => {
    try {
      const response = await fetch(`${path}/api/comments/post/${postId}`);
  ...
  const toggleComments = async (postId) => {
    try {
      if (activePost !== postId) {
        ...
        const postComments = await fetchCommentsOfPost(postId);
        ...
        <div className="flex-1">
          ...
          {comment.replies?.length > 0 && (
            <div className="mt-2 space-y-3">
              {comment.replies.slice(0,4).map((reply) => renderComment(reply, comment._id, true , reply.user.fullname))}
            </div>
          )}
        </div>
      </div>
...
/>
                    <button
                      ...
                      className={`absolute right-3 top-1/2 transform -translate-y-1/2 p-1 rounded-full ${
                        newComment.trim() ? "text-emerald-600" : "text-gray-400"
                      }`}
                    >
                      <FiSend />
                    </button>
